...
// #define DS18B20_PIN_MUX          PERIPHS_IO_MUX_GPIO0_U
// #define DS18B20_PIN_FUNC         FUNC_GPIO0
// #define DS18B20_PIN              0

#define DS18B20_PIN_MUX          PERIPHS_IO_MUX_U0RXD_U
#define DS18B20_PIN_FUNC         FUNC_GPIO3
...
void DS18B20_init(void){
    PIN_FUNC_SELECT(DS18B20_PIN_MUX, DS18B20_PIN_FUNC);
    PIN_PULLUP_EN(DS18B20_PIN_MUX);
...
void write_bit(int bit)
{
  ...
  if (bit) {
      ...
      GPIO_DIS_OUTPUT(DS18B20_PIN);		  // was OW_RLS	// input
      os_delay_us(55);
  }else{
      //55us low
      GPIO_OUTPUT_SET(DS18B20_PIN, 0);
...
int read_bit()
{
  ...
  //Read value
  if (GPIO_INPUT_GET(DS18B20_PIN)){		 //was if (OW_IN)
    bit = 1;
  }
  //Wait for end of bit
  os_delay_us(40);
...
int reset(void)
{
    ...
    GPIO_OUTPUT_SET(DS18B20_PIN, 0); //was OW_LO
    os_delay_us(500);

    //Give up control of the bus and wait for a device to reply
    GPIO_DIS_OUTPUT(DS18B20_PIN); //set to input
    os_delay_us(80);

    //The bus should be low if the device is present:
    if (GPIO_INPUT_GET(DS18B20_PIN)){
        //No device
	return 1;
    }
...
int8_t DS18B20_read(float* temperature) {

    reset();
    write_byte(DS18B20_SKIP_ROM);  // skip ROM command
    write_byte(DS18B20_CONVERT_T); // convert T command

    os_delay_us(750);
    reset();
    write_byte(DS18B20_SKIP_ROM);		// skip ROM command
    write_byte(DS18B20_READ_SCRATCHPAD); // read scratchpad command
